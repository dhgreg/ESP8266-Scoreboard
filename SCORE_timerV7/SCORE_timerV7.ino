...
CRGB solidColor = CRGB::Red;  // Startup light check
uint8_t power = 1;

bool ActiveTeam       = 0;
//timer vars
unsigned long timerOldTime = 0;  //placeholder for multitasking the timer.
int           interval = 100;
long int      startTime = 36000; //the time for the timer in tenths of seconds 1 minute = 600
long int      currentTime;
uint8_t       timerDisplayDelay;

bool          setTimer   = true;  //this is used to check if the timer is set.  True will set the Time / False will skip the time setting loop
...
bool          colonOn    = false;
int8_t        colonCount = 0;
int8_t        timerCorrect;  //Used to correct the poor time keeping  this is a counter
...
//game timer vars
unsigned long gameOldTime = 0;
long int      startGameTime ; //the time for the timer in tenths of seconds 1 minute = 600
int           currentGameTime;
uint8_t       GameDisplayDelay;
bool          setGame   = true;
...
bool showGame       = false;  //shows the game timer
...
int num[11][26] {
  ...
  /*Blank*/ {1}  //This will run the FOR LOOPS with empty values.
};
uint8_t BLANK = 10;  //used to black out leading zeros in the timer

/*********************************************************************
  Setup for the Team varibles
***********************************************************************/
uint16_t teamScore[] = { 0 , 0 };
int8_t teamColor[2][3] = {{247, 19, 19}, {0, 51, 255}};  //starting team colors
...
void setup(void) {
  ...
  server.on("/RS", HTTP_POST, []() { //reset scores (new game)
    teamScore[0] = 0;
    teamScore[1] = 0; 
  });
  ...
  server.on("/showFlare", HTTP_POST,[](){
    falseAll();
    if(showPatterns == true){showPatterns = false; showScores = true;}else{showPatterns = true;}
    showPatterns  = true;
  });
...
void pTimer(){
      fadeToBlackBy( leds, NUM_LEDS, 5);
        hh = currentTime / 36000 % 24;  //REMEBER time is base on tenths not hundreds
        mm = currentTime / 600 % 60;
        ss = currentTime / 10 % 60;
        ms = currentTime  % 10;
  
        if(currentTime>36000){
          timerDigit1=hh/10; timerDigit2=hh%10; timerDigit3=mm/10; timerDigit4=mm%10;}
        if(currentTime<=35999 && currentTime>5999){
          timerDigit1=mm/10; timerDigit2=mm%10; timerDigit3= BLANK; timerDigit4= BLANK;}
        if(currentTime<=5998 && currentTime>1230){
          timerDigit1= BLANK; timerDigit2=mm%10; timerDigit3= BLANK; timerDigit4= BLANK;}
        if(currentTime<=1229 && currentTime>599){
          timerDigit1= BLANK; timerDigit2=mm%10; timerDigit3= ss/10; timerDigit4= ss%10;}  
        if(currentTime<=598 && currentTime>=100){
          timerDigit1=ss/10; timerDigit2=ss%10; timerDigit3= BLANK; timerDigit4= BLANK;}
        if(currentTime<=99){
          timerDigit1= BLANK; timerDigit2=ss%10; timerDigit3=ms; timerDigit4= BLANK;}
        if(currentTime<=0){
          pauseTimer = true; timerDisplayDelay++;}
        if(timerDisplayDelay == 40){
            falseAll();
            blackout = true;
          }
        if(timerDisplayDelay>=50){setTimer = true;}
fill_solid(leds, NUM_LEDS, CRGB::Black);  //Clearing out the Colors from the Array 
...
void TimerDisplay(int8_t score, int8_t digit) {
  ...
  for (int8_t i = 1 ; i < s; i++) {                              //loop to set LED colors of the "tens" place. d1 means digit 1
    int16_t k = num[score][i] + jDigitOffset[digit];            // Setting the correct pixel and the group offset
      if(pauseTimer == false || pauseGame == false){
          if(colonCount >=10  ){colonOn = true;}
          if(colonCount >=12){colonOn = false;  colonCount = 0;}
     ...
     if(score >=0 && score <=9){
      leds[k] = CRGB(teamColor[ActiveTeam][0], teamColor[ActiveTeam][1], teamColor[ActiveTeam][2]);
     }else{
      leds[k] = CRGB(0,0,0);
      }
     }
...
//Timer
  ...
  if(millis() - timerOldTime >= interval) {
    timerOldTime = millis();
    if(pauseTimer == false){if(currentTime < 100){colonCount = colonCount +3;}else{colonCount++;}}
    if(pauseTimer == false && startTimer == true ){ 
      currentTime--; 
      timerCorrect++; 
      if(timerCorrect >= 16){currentTime--; timerCorrect = 0;} // Used to correct bad time keeping.  should use a RTC for better time but we will cheat with this instead.
    }
    if( showTimer == true){ pTimer();}
  }  
//pattern
  if (showPatterns == true) {pride();}
//blackout
...
if(showFlare == true){showFlareFunc();}  //shows a little Flare with each 5 giving in the judging
...
void falseAll(){
  blackout       = false;  
  showFlare      = false;  //add a little flare to the numbers
  showPatterns   = false;  //shows the patterns or if false shows the timers or score
  showJudges     = false;  // Shows the Judges Scores
  showScores     = false;   //shows the Team scores
  showGame       = false;  //shows the game timer
  showTimer      = false;
  }
